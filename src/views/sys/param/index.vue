  ...
  <div class="app-container">
    ...
    <div>
      <el-dialog :title="dialogType === 'edit'?'编辑':'新增'" :visible.sync="dialogVisible" width="30%">
        <el-form :model="updateParam" label-width="80px" label-position="left">
          <el-form-item label="参数编码">
            <el-input v-model="updateParam.code" auto-complete="off" placeholder="参数编码" />
          </el-form-item>
          <el-form-item label="参数值">
            <el-input v-model="updateParam.value" auto-complete="off" placeholder="参数值" />
          </el-form-item>
          <el-form-item label="备注">
            <el-input v-model="updateParam.note" :autosize="{ minRows: 2, maxRows: 4}" type="textarea" placeholder="备注" auto-complete="off" />
          </el-form-item>
        </el-form>
...
<script>
import { getParamList, updateParam, deleteParam, deleteListParam, getAccountListByParamId } from '@/api/param'
...
export default {
  ...
  methods: {
    ...
    getList() {
      getParamList(this.listQuery).then(response => {
    ...
    add() {
      ...
      this.updateParam = Object.assign({}, defaultupdateParam)
      this.dialogType = 'new'
    },
    modifyRow(scope) {
      this.dialogVisible = true
      this.dialogType = 'edit'
      this.updateParam = deepClone(scope.row)
    },
    ...
    async deleteData(scope) {
      await deleteParam(scope.row.id)
    ...
    async confirm() {
      await updateParam(this.updateParam)
      this.getList()
